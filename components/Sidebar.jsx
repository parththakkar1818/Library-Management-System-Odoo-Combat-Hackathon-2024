...
const Sidebar = () => {
        ...
        return (
            <Link 
                ...
                className={clsx(
                    ...
                    path === el.link.split("/")[0] ? "bg-blue-700 text-white hover:text-black" : ""
                )}
            >
                {el.icon} 
    ...
    const isL = true; // Change this to false to test the other condition

    const filteredLinks = isL 
        ? linkData.filter(link => ["dashboard", "books", "users/users", "borrowed/borrowed"].includes(link.link))
        : linkData.filter(link => ["books", "user/books", "fine/fine"].includes(link.link));
