...
const DashBoard = () => {
  ...
  const booksVsCategoryData = [
    ...
    { category: 'Science Fiction', count: 30 },
    ...
    <div className="container mx-auto p-4">
      ...
      <p className="font-bold"> Visitors & Borrowers</p>
        <BarChart
          ...
          xAxis={[{ scaleType: 'band', data: data1.visitors.map(item => item.day) }]}
          series={[
            { data: data1.visitors.map(item => item.count), name: 'Visitors' },
            { data: data1.borrowers.map(item => item.count), name: 'Borrowers' }
          ]}
          legends={[
      ...
      <p className="font-bold">Category & Number of Books</p>
        <BarChart
          ...
          xAxis={[{ scaleType: 'band', data: booksVsCategoryData.map(item => item.category) }]}
          series={[{ data: booksVsCategoryData.map(item => item.count), name: 'Books' }]}
          legends={[
            { data: 'Books', color: '#4caf50' }
          ]}
          width={600}
          height={400}
