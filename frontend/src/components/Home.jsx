...
const Home = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await fetch("http://localhost:8000/get_trending_newarrivals"); // Single endpoint for both new arrivals and trending books
        if (!response.ok) {
  ...
  const renderSmallItems = () =>
    ...
    const renderBooks = (books) =>
      books.map((book, index) => (
        <div
          key={index}
          className="book-item p-4 border border-gray-200 rounded-lg shadow-md flex"
        >
          <img
            src={book.thumbnail}
            alt={book.book_name}
